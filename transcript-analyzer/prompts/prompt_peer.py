...
**5. 数字规范：**
   ...
   - 增速变化方向必须标注：加速↑ / 减速↓ / 持平→
   - 利润率对比用绝对值 + 环比变化 (pp)
   - 数据可靠性标签：关键对比数据须标注 `[H]` Hard（管理层明确数字）/ `[D]` Derived（推导）/ `[I]` Inferred（推断）。跨公司对比时，可靠性不一致的数据点必须注明（如"A 公司 [H] vs B 公司 [I]，对比需谨慎"）
...
| [竞争领域名称] | {table_header} |
|:---|{table_separator}|
| 该业务营收 | [值+YoY%] (p.X) `[H]`/`[D]`/`[I]` | [值+YoY%] (p.X) `[H]`/`[D]`/`[I]` | ... |
| 增速方向 | 加速↑/减速↓ | ... | ... |
| 该业务利润率 | X% (p.X) `[H]`/`[D]`/`[I]` | X% (p.X) `[H]`/`[D]`/`[I]` | ... |
